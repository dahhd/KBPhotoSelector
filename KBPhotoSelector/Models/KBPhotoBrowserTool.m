...
@implementation KBPhotoAlbumList

@end

//--------------------------------------------------------------------------------------------------

@implementation KBPhotoBrowserTool

static KBPhotoBrowserTool *sharedPhotoTool = nil;

+ (instancetype)sharedPhotoBrowserTool {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedPhotoTool = [[KBPhotoBrowserTool alloc]init];
    });
    return sharedPhotoTool;
}
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedPhotoTool = [super allocWithZone:zone];
    });
    return sharedPhotoTool;
}
...
{
    NSMutableArray<KBPhotoAlbumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            ...
            if (assets.count > 0) {
                KBPhotoAlbumList *ablum = [[KBPhotoAlbumList alloc] init];
                ablum.albumTitle = collection.localizedTitle;
                ablum.albumCount = assets.count;
                ablum.albumFirstImageAsset = assets.firstObject;
                ablum.albumCollectionAsset = collection;
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *photosBytes))completion
{
    ...
    for (int i = 0; i < photos.count; i++) {
        KBSelectPhotoModel *model = photos[i];
